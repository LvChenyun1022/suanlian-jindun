...
申报赛道三“金融行业场景落地创新”的 **GPU 融资租赁材料审查与跨案风险核验智能体**。
当前版本是可审计工具型风控工作流（审计智能体原型）：按确定性流程调用三单解析、规则核验、
跨案查重、示意性压力测试、评分路由与审计打包工具。它不是自主授信系统，也未完成生产部署。
...
- 生产业务侧的外部负面信号与动产登记查询仍为 mock/可替换接口；本次 live 评测只在
  “纯 LLM 单案直判”消融基线上调用第三方兼容网关，不代表已接入生产外部数据。

系统边界详见 [SPEC.md](SPEC.md) 第 1 节；监管条款到产品功能的对照见 [docs/compliance.md](docs/compliance.md)。

## 参赛主体与责任边界

旧版赛事文件要求 2–5 人组队；2026-09-02 最新官方公告允许个人或 1–5 人组队。项目负责人已向
赛事方确认可按个人形式参赛；确认记录由负责人私下留存，不在公开仓库披露沟通截图或个人联系方式。项目负责人独立承担需求定义、
...
可选依赖（`requirements-optional.txt`，不安装不影响运行）：`langgraph`（实验性直线编排，
当前不声称与主流程完全等价或具备自主规划）、`paddleocr`（仅外部评测脚本使用，尚未接入主流程）。
...
| 三单核验 F1 | 0.9091 | ≥0.90 | ✅ |
| 合成集已知模式召回 / 合成正常样本误报率 | 83.33%（25/30）/ 0.00%（0/70；上界约 4.3%） | ≥90% / ≤10% | ❌ / ✅ |
...
| 消融：信息 + 工具链召回增量 | **+50.0pp**（系统 83.33% vs mock 关键词基线 33.33%） | ≥15pp | ✅ |

> 上述结果均来自合成闭集或固定回归集，不作为生产误报率、真实欺诈识别或普遍安全能力结论。

### live 模式（100 案正式重跑，2026-08-23，[eval/results_live_temporal_20260823/eval_results.md](eval/results_live_temporal_20260823/eval_results.md)）

主系统采用上述修正后口径（正则优先设计，pipeline 全程 0 LLM 调用、0 token、成本 0 元/案）。
真实 LLM 直判基线采用第三方 OpenAI-compatible 网关 `https://yuanyuaicloud.cn/v1`、
`deepseek-v4-pro`，沿用 `eval/baseline.py` v2-fixed-2026-08-11（temperature=0，严格 JSON 输出）。
100 案均返回有效结构化结果：**306,201 tokens、0 invalid、0 baseline errors**；主系统仍为
0 LLM 调用、0 token。此次命令未传 `--price-per-1k-tokens`，故只报告 token，不把 0 元误写成
基线真实费用。live 主系统端到端时耗为均值 0.157s、最大 0.267s。
...
**历史结果与基线有效性复核（如实记录）**：首轮 live 基线（v1）曾退化为"逢案必报"（召回 100%/误报 100%，
提升 +0.0pp）。复核发现 v1 存在子串解析脆弱、异常默认映射、低门槛诱导措辞等 6 项实现 bug
（历史复核详见 `eval/results_live/eval_results_after_baseline_fix.md` 与
...
修复前原始结果保留于 `eval/results_live/eval_results_before_baseline_fix.{json,md}`；这些文件是
历史审计证据，不再作为当前正式成绩。
...
- `case_level_rule_set_exact_match`：95.00%，低于 100% 目标。兼容字段 `rule_accuracy`
  暂时保留；对应同一结构性原因：5 个首次出现的
...
三单 PDF ──► 护栏（注入检测/敏感数据拒绝/工具白名单）
        ...
        ──► GPU 示意性压力测试计算器（未经真实租金计划/债务偿付表校准）
        ──► 利用率预警（绿/黄/红，"T-N 天预警"）
...
- **纯 Python 顺序 pipeline 为主**（`src/pipeline.py`，state 为 SPEC 定义的 Pydantic 模型）；
  当前是确定性工作流 + 受限路由，LangGraph 文件仅为实验性直线编排；
...
- GPU 模块是示意性压力测试计算器：合同总额近似租金计划，残值与剩余本金共用折旧曲线，
  LTV/DSCR 不构成定价、估值、授信或偿债能力判断；
...
完整结果：[eval/results/external_validity.md](eval/results/external_validity.md)。原始外部文件因许可与
敏感信息限制不随仓库分发；仓库仅提供来源与标注清单，评委取得相同公开文件后方可复跑。
...
（取得相同公开文件后运行：`python -m eval.run_external --ocr`；OCR 默认关闭，仅 `--ocr` 或
...
- EVAL-01：取消隐式加载全量标签，评测按时间序仅用前案系统输出构建登记上下文；修正后召回
  83.33%、规则准确率 95.00%、mock/2026-08-23 正式 live 提升均为 +50.0pp。
...
- [docs/plan_final.pdf](docs/plan_final.pdf) — 参赛项目书提交版（16 页 PDF，以此版式为准）
- [docs/plan_final.docx](docs/plan_final.docx) — 参赛项目书可编辑版（不同办公软件可能重新分页）
- [docs/submission_manifest.md](docs/submission_manifest.md) — 冻结文件哈希与赛前核验清单
