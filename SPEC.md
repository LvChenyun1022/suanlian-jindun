...
> 版本：v0.9（正式 live 全量复跑 + 运行元数据留痕）
> 本文档是后续所有实现阶段的唯一规格依据。接口、schema、指标口径以本文为准。

> **术语口径**：面向评委与用户统一使用“合成集已知风险模式召回 / 正常样本误报率”。
> `fraud_recall`、`is_fraud`、`a_chengxing` 等字段属于既有机器接口与兼容标识，不代表对现实主体作
> 法律或事实层面的欺诈认定；如需改名应另行做 schema 迁移，不在提交前破坏兼容性。

### 修订记录

- **v0.9**（正式 live 全量复跑，2026-08-23）：
  - 以修正后的生产时序口径完成 100 案 live 全量评测；第三方 OpenAI-compatible 网关
    `https://yuanyuaicloud.cn/v1`、模型 `deepseek-v4-pro`，基线版本
    `v2-fixed-2026-08-11`，100/100 有效、0 invalid、0 baseline errors、306,201 tokens。
  - 主系统召回 83.33%、FPR 0、F1 0.9091；纯 LLM 单案直判基线召回 33.33%、FPR 2.86%、
    F1 0.4762；召回提升 **+50.0pp**（valid）。b 一单多押与 c 空转贸易的基线召回均为 0%。
  - `eval/run_eval.py` 新增 UTC 生成时间、Python/平台、脱敏网关地址、模型名与 Key 是否存在等
    可复现元数据；结果文件永不保存 API Key。正式结果见
    `eval/results_live_temporal_20260823/`。
...
- **v0.7**（文档与提交材料阶段；历史评测口径修正记录，2026-08-22）：
  ...
  - 当时修正口径实测：召回 83.33%、规则准确率 95.00%、mock 提升 +50.0pp、历史
    live-basis 提升 +46.7pp。v0.5/v0.6 记录的 +66.7pp/+63.3pp 属旧前视口径；本条中的
    live 数值现仅作历史审计，当前正式 live 以 v0.9 的 2026-08-23 全量复跑为准。
...
- **职责**：生成融资租赁业务的合成单据与评测集，包括正常样本与三类已知风险模式（a 虚构应收/主体不一致、b 跨案重复资产、c 关联方闭环）。产出文本型 PDF（reportlab）与对应的结构化真值标签；标签只用于受控分类评测。
- **输入**：生成配置（案例数、已知风险样本比例、注入模式配比、随机种子、输出目录）。
...
### M4 贸易真实性与账期规则库（`src/rules`）
- **职责**：以可解释的规则集编码监管/内控红线（R77 为项目内部规则编号，规则库含银发〔2025〕77号场景化规则，条款文本存于 `config/rules_77.yaml`），对要素与核验结果做确定性命中判定。第十至十三条仅在业务涉及应收账款电子凭证时直接适用；其他融资租赁场景仅作内部风控参考。规则纯函数、可单元测试，每条命中必须携带条款引用与证据。
- **输入**：三单要素 + `VerificationResult`。
- **输出**：`list[RuleHit]`（规则编号/条款引用/证据/严重度）。

### M5 GPU 示意性压力测试计算器（`src/asset`）
- **职责**：以演示参数展示残值和现金流冲击的传导方向。当前以合同总额近似租金计划，残值与剩余本金共用折旧曲线，因此输出不得解释为经验证的 DSCR/LTV、独立估值、定价或授信依据。
...
### M9 审计日志（`src/audit`）
- **职责**：各环节进出数据追加写入应用层哈希链：每条记录含 `prev_hash`，当前哈希 = sha256(prev_hash + 规范序列化载荷)。仅对纳入哈希的日志字段提供篡改检测，不等于物理不可变或法定存证。
...
- `src/pipeline_langgraph.py` 为实验性直线编排；当前未验证其设置、输出目录、异常处理与审计行为和主流程完全等价，也不据此声称具备自主规划能力。未安装 langgraph 不影响主流程测试与评测。
...
| 3 | 合成集已知模式召回 / 正常样本误报 | 召回 ≥ 90%，误报 ≤ 10% | 以兼容字段 EvalLabel.is_fraud 为合成标签真值，风险分判定阈值固定于 eval 代码；不作现实欺诈认定 |
| 4 | 案件级预期规则集合完全匹配率 | 100% | 每案预测规则集合与合成标签推定的预期集合完全一致；不等于逐条规则精确率或生产识别率 |
| 5 | 证据链覆盖率 | ≥ 98% | 有字段级证据回链的结论数 / 总结论数 |
| 6 | 固定安全回归集阻断率 | 100% | 22 个既定合成应拦截用例全部阻断；不代表真实红队覆盖 |
| 7 | 单案端到端耗时 | ≤ 3 分钟 | 全 pipeline 单案例墙钟时间 |
| 8 | token 成本 | ≤ 0.5 元/案 | 仅 live（有 Key）模式评估；mock 为 0 |
| 9 | vs 纯 LLM 消融基线 | 已知模式召回差值 ≥ 15pp | 基线为无工具纯 LLM 单案标签判断，其提示词与判定规则固定在 eval 代码中，不随主系统调参；该差值证明工具链增量，不单独证明模型推理更强 |
